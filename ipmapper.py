...
#Source: https://stackoverflow.com/questions/477486/python-decimal-range-step-value
def frange(x, y, jump):
    while x < y:
        yield x
        x += jump
...
def make_map(pkts):
    ...
    publicips = {ip: count for (ip, count) in ipdict.iteritems() if IP(ip).iptype()=='PUBLIC'}
    ...
    circle = Circle(x="lon", y="lat", size="size", fill_color="fill", fill_alpha=0.7,line_color="black")
